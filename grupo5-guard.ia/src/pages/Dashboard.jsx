...
export default function Dashboard() {
  ...
  const [metrics, setMetrics] = useState({
    totalProposicoes: 12,
    totalCategorias: 7,
    totalParlamentares: 12,
    totalEstados: 10,
    porCategoria: [
      { nome: "Proteção Digital", qtd: 4 },
      { nome: "Redes Sociais", qtd: 3 },
      { nome: "Educação Digital", qtd: 3 },
      { nome: "Cyberbullying", qtd: 1 },
      { nome: "Direitos Digitais", qtd: 1 }
    ],
    evolucao: [
      { name: 'Jan', volume: 4 },
      { name: 'Fev', volume: 7 },
  ...
  useEffect(() => {
    // Simula o esqueleto de carregamento profissional por 600ms antes de renderizar
      ...
      <main style={{ padding: '40px 48px' }}>
        ...
        <section style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px', marginBottom: '32px' }}>
          {[
            ...
            { title: 'PARLAMENTARES', value: metrics.totalParlamentares },
            { title: 'ESTADOS', value: metrics.totalEstados }
          ].map((card, i) => (
            <div key={i} style={{ background: 'var(--surface)', padding: '24px', borderRadius: 'var(--radius)', boxShadow: 'var(--shadow)', border: '1px solid var(--border)' }}>
              ...
              {loading ? <div className="skeleton" style={{ height: '38px', width: '60px', marginTop: '4px' }}></div> : <span style={{ fontSize: '36px', fontWeight: '700', color: 'var(--primary)' }}>{card.value}</span>}
            </div>
          ))}
        </section>
        ...
        <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
              ...
              metrics.porCategoria.map((cat, idx) => (
                <div key={idx} style={{ display: 'flex', alignItems: 'center', marginBottom: '14px', gap: '16px' }}>
                  ...
                  <div style={{ flex: 1, backgroundColor: '#f1f5f9', height: '12px', borderRadius: '9999px', overflow: 'hidden' }}>
                    <div style={{ backgroundColor: 'var(--primary)', height: '100%', borderRadius: '9999px', width: `${(cat.qtd / 4) * 100}%`, transition: 'width 1s ease' }}></div>
                  </div>
          ...
          {/* Gráfico 2: Evolução Temporal com Recharts Real */}
          <div style={{ background: 'var(--surface)', padding: '28px', borderRadius: 'var(--radius)', boxShadow: 'var(--shadow)', border: '1px solid var(--border)' }}>
            <h4 style={{ margin: '0 0 20px 0', fontSize: '15px', color: 'var(--text-main)', fontWeight: '700' }}>Evolução Temporal de Capturas</h4>
            <div style={{ width: '100%', height: '200px' }}>
              <ResponsiveContainer>
                <AreaChart data={metrics.evolucao} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  <defs>
